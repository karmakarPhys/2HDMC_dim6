...
double DecayTable::get_gamma_hdd(int h, int d1, int d2) {
  ...
  complex <double> cs,cp;
  model.get_coupling_hddp(h,d1,d2,cs,cp);
...
double DecayTable::get_gamma_huu(int h, int u1, int u2) {
  ...
  complex <double> cs,cp;
  model.get_coupling_huup(h,u1,u2,cs,cp);
...
double DecayTable::get_gamma_hll(int h, int l1, int l2) {
  ...
  complex <double> cs,cp;
  model.get_coupling_hllp(h,l1,l2,cs,cp);
...
// Note that there is a typo in Djouadis review for the last term! (40/9 instead of 40/27)
  ...
  //double KHVV = 3.*pow(c2,2)*GF/(64.*sqrt(2.)*pow(M_PI,3))*M*dV;
...
double DecayTable::hvv_offshellp(int h, int V, double M) {
  double c = DecayTable::hvv_offshell(h, V, M);
  double a; 
  double sbap = model.get_sbap(); double cbap = model.get_cbap();
  if(h==1){a = pow(sbap,2);}
  else if (h==2) {a = pow(cbap,2);}
  else if (h==3) {a =1;}
  double G=c*pow(a,2);
  return G;
}
...
/*double DecayTable::hvv_allp(int h, int V) {
 double M = 125;
  double G = DecayTable::hvv_all(h, V, M);
  double a; 
  double sbap = model.get_sbap(); double cbap = model.get_cbap();
  if(h==1){a = pow(sbap,2);}
  else if (h==2) {a = pow(cbap,2);}
 ...
 complex <double> DecayTable::hvv_coup_sq(int V, int h){
  complex <double> c;
  model.get_coupling_vvh(V,V,h,c);
  //complex<double> xxx = c;
  //return xxx;
  return c;
}
...
double DecayTable::hvv_onshellp(int h, int V, double M) {
  double c = DecayTable::hvv_onshell(h, V, M);
  double a; 
  double sbap = model.get_sbap(); double cbap = model.get_cbap();
  if(h==1){a = pow(sbap,2);}
  else if (h==2) {a = pow(cbap,2);}
...
double DecayTable::hff_onshell(double M, double m1, double m1run, double m2, double m2run, complex<double> cs, complex<double> cp, int Nc, int h, bool tt=false) {
  ...
  model.get_coupling_huup(h,3,3,cst,cpt);
...
double DecayTable::htt_onshell(double M, double m1, double m2,  int Nc, int h) {
  ...
  model.get_coupling_huup(h,3, 3,cs,cp);
...
double DecayTable::hgaga(int h) {
  ...
  // Down-type quarks
  for (int i=1;i<=3;i++) {
    model.get_coupling_hddp(h,i,i,gS,gP);
  ...
  // Up-type quarks
  for (int i=1;i<=3;i++) {
    model.get_coupling_huup(h,i,i,gS,gP);
...
// QCD corrections in heavy quark limit for CP-even Higgs
  ...
  // Leptons
  for (int i=1;i<=3;i++) {
    model.get_coupling_hllp(h,i,i,gS,gP);
